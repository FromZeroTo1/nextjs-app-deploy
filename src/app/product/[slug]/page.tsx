...
async function getProduct(params: TypeParamSlug) {
	...
	const additionalProducts = await ProductService.getAll({ perPage: 10 })

	return { product, similarProducts, additionalProducts }
}
...
export default async function ProductPage({ params }: IPageSlugParam) {
	const { product, similarProducts, additionalProducts } =
	...
	return (
		<Product
			...
			additionalProducts={additionalProducts.products}
			slug={params.slug}
		/>
	)
}
